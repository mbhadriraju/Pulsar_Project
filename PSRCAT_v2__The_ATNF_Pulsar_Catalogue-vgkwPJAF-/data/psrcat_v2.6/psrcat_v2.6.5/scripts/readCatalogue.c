...
int readCatalogue(pulsar *psr,linkedList *list,int *npsr,
                  char name[MAX_PSR][MAX_NAMELEN], int iname,int *haveName,
                  char *filterStr, int *ptr, char sptr[MAX_PLIST][MAX_STRLEN],
                  int np,
                  char *dbFile,int merge,int willmerge,
                  char *boundary,
                  paramtype *defParam,
                  int checkCat,
                  char alias1[MAX_ALIASES][MAX_STRLEN],
                  char alias2[MAX_ALIASES][MAX_STRLEN],
                  unsigned int aliasCount,int exactMatch)
{
  FILE *fin;
...
int rnd8(double rval,double rerr,int ifac,char *cval,int *lv,char *cerr,int *le,char *msg)
{
  ...
  /* avoid -inf, which leads to integer underflow */
  if (ee > epsilon) {
      xe = log10( ee/2.0 ) + epsilon;
  } else {
    printf("ERROR: should not be here, in readCatalogue.c\n");
    exit(1);
    xe = epsilon;
  }
  ixe = fabs(xe);
