...
N = 10
tensor = np.zeros((N, 64, 2048), dtype=np.float32)
...
for j in range(N):
    try:
        ...
        gauss_prof.init_profiles(2048, Nchan=Nf)
        ...
        signal_min = signal_fold_data.min()
        signal_max = signal_fold_data.max()
        signal_fold_data = (signal_fold_data - signal_min) / (signal_max - signal_min)
        tensor[j] = signal_fold_data
    except IndexError:
        j -= 1
        continue
